 ...
 * 帖子评论区（两层模型，乐观发布）。
 *
 * <p>未登录展示 gate；登录后拉顶层评论（倒序分页，含 reply_count），支持乐观发布顶层评论、
 * 展开回复列表与回复表单、作者删除（软删占位）。写操作先乐观插入本地，再异步请求，
 * 失败回滚 + 瞬时提示。
 */
export function CommentSection({ postId }: { postId: string }) {
  ...
  const loadPage = useCallback(
    (pageToLoad: number, append: boolean) => {
      ...
      commentsApi
        ...
        .then((res) => {
          const items = (res.content ?? []) as CommentView[];
  ...
  const submit = useCallback(() => {
    ...
    setPosting(true);
    run(
      () => commentsApi.create(postId, { content }),
      {
        // 乐观：立即插入本地；空态需切到 content 才会渲染列表
        optimistic: () => {
          setComments((prev) => [optimistic, ...prev]);
          setPhase("content");
        },
        // 列表语义回滚：仅移除本条临时项（不动其它已存在/成功的评论），相位还原到提交前
        rollback: () => {
        ...
        onOk: (created) => {
          setPosting(false);
          setComments((prev) => prev.map((c) => (c.id === tempId ? (created as CommentView) : c)));
        },
        onError: (err) => {
          setPosting(false);
    ...
    setDraft("");
  }, [postId, requireAuth, draft, phase, user, run]);
